...
class InitializationManager {
    constructor() {
        ...
        console.logDebug('🔧 Initialization Manager created');
    }
    ...
    registerComponent(name, instance = null) {
        if (this.componentStatus.hasOwnProperty(name)) {
            ...
            console.logDebug(`✅ Component registered: ${name}`);
            
            // Check if we can proceed with dependent initializations
            this.checkDependencies();
        } else {
            console.logWarn(`⚠️ Unknown component registration: ${name}`);
        }
    }
    ...
    waitForComponents(componentNames, callback, timeout = 10000) {
        ...
        const pollInterval = setInterval(() => {
            if (checkReady()) {
                ...
                clearInterval(pollInterval);
                console.logWarn(`⚠️ Timeout waiting for components: ${componentNames.join(', ')}`);
    ...
    async startInitialization() {
        ...
        console.logInfo('🚀 Starting system initialization sequence...');
        ...
        try {
            ...
            console.logInfo('✅ System initialization complete');
    ...
    async initializePhase1() {
        console.logDebug('📋 Phase 1: Checking basic dependencies...');
    ...
    async initializePhase2() {
        console.logDebug('📋 Phase 2: Initializing core systems...');
    ...
    async initializePhase3() {
        console.logDebug('📋 Phase 3: Initializing enhanced features...');
    ...
    async initializePhase4() {
        console.logDebug('📋 Phase 4: Final integration checks...');
    ...
    createSafeAccessor(globalName, fallback = null) {
        return () => {
            ...
            console.logWarn(`⚠️ ${globalName} not available, using fallback`);
            return fallback;
        };
    }
    ...
    checkDependencies() {
        ...
        console.logDebug(`🔍 System readiness: ${status.readinessPercentage}% (${status.readyComponents}/${status.totalComponents})`);
        
        if (status.missingComponents.length > 0) {
            console.logDebug('⏳ Still waiting for:', status.missingComponents.join(', '));
        }
    }
}
...
console.logInfo('✅ Initialization Manager loaded');