...
class AuthWrapper {
    ...
    // Open login/signup modal (compatibility method)
    open(type = 'login') {
    ...
    showLoginForm() {
        ...
        modal.innerHTML = `
            <div class="auth-modal-content">
                ...
                <p>Don't have an account? <a href="#" onclick="authWrapper.open('signup')">Sign up</a></p>
            </div>
        `;
    ...
    showSignupForm() {
        ...
        modal.innerHTML = `
            <div class="auth-modal-content">
                ...
                <p>Already have an account? <a href="#" onclick="authWrapper.open('login')">Login</a></p>
            </div>
        `;
        ...
        document.getElementById('auth-signup-form').onsubmit = async (e) => {
            ...
            const password = document.getElementById('auth-password').value;
            try {
                await this.signup(email, password, { name: name });
...
function initializeAuthWrapper() {
    ...
    // Optional: Create alias for easier migration
    window.netlifyIdentity = window.authWrapper;
}

// If Supabase is already initialized, create wrapper immediately
if (window.SupabaseAuth) {
    initializeAuthWrapper();
} else {
    // Wait for Supabase to be ready
    window.addEventListener('supabaseReady', initializeAuthWrapper);
}